...
function countdown(start, stop){
	//get miliseconds when subtract 2 times
	time = Math.abs(start - stop);
	days = time/1000/60/60/24;
	hours = time/1000/60/60%24;
	mins = (hours - Math.floor(hours))*60;
	seconds = (mins - Math.floor(mins))*60;
	
	return (Math.floor(days) + " days, " + Math.floor(hours) + " hrs, " + Math.floor(mins) + " mins, " + Math.ceil(seconds) +" secs");
}